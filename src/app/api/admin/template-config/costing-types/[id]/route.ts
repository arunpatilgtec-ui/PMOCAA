...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (typeof data.code === 'string' && data.code.trim()) update.code = data.code.trim().toUpperCase()
    if (typeof data.label === 'string') update.label = data.label.trim()
    if (typeof data.order === 'number') update.order = data.order

    const costingType = await prisma.templateCostingType.update({ where: { id }, data: update })
    return Response.json(costingType)
  } catch (err) {
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireAdmin()
    const { id } = await params
    await prisma.templateCostingType.delete({ where: { id } })
